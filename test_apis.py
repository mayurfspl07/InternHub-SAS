...
def run(base, admin_email="admin@internhub.dev", admin_password="Imp@pune1"):
    ...
    if not test("POST /api/auth/login", r):
        return False
    data = r.json()
    ...
    test("GET /api/admin/students (no auth) -> 403", r, expected=403)

    # 4. Students list
    r = s.get(f"{base}/api/admin/students", timeout=15)
    if not test("GET /api/admin/students", r):
        return False
    d = r.json()
    total = d.get("total", 0)
    ...
    if first_id:
        ...
        if test(f"GET /api/admin/students/{first_id}/attendance", r):
            d = r.json()
            totals = d.get("totals", {})
        ...
        if test(f"GET /{first_id}/attendance?month=2026-08", r):
            d = r.json()
        ...
        if test(f"GET /{first_id}/attendance?status=present", r):
        ...
        if test(f"GET /{first_id}/attendance?start=2026-08-01&end=2026-08-21", r):
